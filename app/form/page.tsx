...
export default function FormPage() {
        ...
        if (idParam) {
            const fetchRecord = async () => {
                try {
                    ...
                    if (!isNaN(id)) {
                        const record = await db.medical_records.get(id);
                        if (record) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // Usually after save, we go back to list
            router.push('/list');
        ...
        <div className="min-h-screen bg-slate-50/80 pb-32 font-[family-name:var(--font-geist-sans)]">
            ...
            <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-full px-6 flex justify-center z-50 pointer-events-none">
                <div className="bg-slate-900/90 backdrop-blur-md text-white px-2 py-2 rounded-2xl shadow-2xl border border-slate-700/50 flex items-center gap-2 pointer-events-auto scale-90 md:scale-100 transform transition-transform">
                    <button onClick={() => router.push('/dashboard')} className="p-4 hover:bg-white/10 rounded-xl transition-colors group relative">
                    ...
                    <div className="w-px h-8 bg-white/10"></div>
                    <button onClick={() => router.push('/list')} className="p-4 hover:bg-white/10 rounded-xl transition-colors group relative">
                        <span className="text-xl">📂</span>
